...
pub fn console_box(lines: &[&str], title: Option<&str>) {
    ...
    let max_len = lines.iter().map(|line| line.len()).max().unwrap().max(title.unwrap_or("").len());
    ...
    if let Some(title) = title {
        ...
        box_lines.push(format!("{}{}{}{}{}{}", left_space, green.apply_to(box_tl), green.apply_to(left), cyan.apply_to(title), green.apply_to(right), green.apply_to(box_tr)));
    } else {
        box_lines.push(format!("{}{}{}{}", left_space, green.apply_to(box_tl), green.apply_to(box_h.repeat(width_offset)), green.apply_to(box_tr)));
    }

    for i in 0..height {
        if i < valign_offset || i >= valign_offset + lines.len() {
            box_lines.push(format!("{}{}{}{}", left_space, green.apply_to(box_v), " ".repeat(width_offset), green.apply_to(box_v)));
        } else {
            let line = lines[i - valign_offset];
            let left = " ".repeat(padding_offset);
            let right = " ".repeat(width - line.len());
            box_lines.push(format!("{}{}{}{}{}{}", left_space, green.apply_to(box_v), left, cyan.apply_to(line), right, green.apply_to(box_v)));
        }
    }

    box_lines.push(format!("{}{}{}{}", left_space, green.apply_to(box_bl), green.apply_to(box_h.repeat(width_offset)), green.apply_to(box_br)));
